...
export function ProgressDetail({ track: t }: { track: Track }) {
  ...
  const indeterminate = t.progress <= 0 && info.phase !== 'download';
  ...
  return (
    <div className="mt-2 space-y-1.5">
        ...
        {PHASES.map((p, i) => {
          ...
          return (
            <li key={p.id} className="flex items-center gap-1 min-w-0">
              <span
                className={`px-1.5 py-0.5 rounded-md whitespace-nowrap transition ${
                  state === 'active' ? 'bg-accent-soft text-white ring-1 ring-accent/60' : state === 'done' ? 'text-zinc-400' : 'text-zinc-600'
                }`}
              >
                {state === 'done' ? '✓ ' : ''}{p.label}
                {state === 'active' && info.step ? <span className="tabular-nums text-zinc-300"> {info.step.n}/{info.step.total}</span> : null}
              </span>
      ...
      {/* bar */}
      <div className="h-1.5 rounded-full bg-ink-700 overflow-hidden relative">
        <div
        ...
        {info.step && (
          <div className="absolute inset-0 pointer-events-none">
            {[0.25, 0.5, 0.75].map((f) => <span key={f} className="absolute top-0 h-full w-px bg-ink-900/60" style={{ left: `${f * 100}%` }} />)}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-[11px] text-zinc-400 tabular-nums">
        ...
        <span className="shrink-0 flex items-center gap-2">
          <span className="text-zinc-300">{indeterminate ? '' : `${pct}%`}</span>
          {typeof t.elapsed === 'number' && t.elapsed > 0 && <span title="elapsed">{fmtTime(t.elapsed)} elapsed</span>}
          {t.eta !== null && t.eta > 0 && <span title="estimated remaining" className="text-zinc-300">{fmtEta(t.eta)}</span>}
        </span>
      </div>
    </div>
  );
